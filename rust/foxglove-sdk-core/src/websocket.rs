...
pub use crate::websocket::protocol::server::{
    Capability, Parameter, ParameterType, ParameterValue,
};
...
struct ConnectedClient {
    ...
    /// Used to cancel the connection from different contexts
...
impl Server {
    ...
    pub async fn start(&self, host: &str, port: u16) -> Result<String, FoxgloveError> {
        ...
        self.runtime_handle
            .spawn(handle_connections(self.arc(), listener));
    ...
    pub fn start_blocking(
        ...
        self.runtime_handle.block_on(self.start(host, port))
    }
    ...
    pub async fn stop(&self) {
        ...
        self.clients.clear();
        tracing::info!("Finished shutting down");
    }

    pub fn stop_blocking(self: &Arc<Self>) {
        self.runtime_handle.block_on(self.stop());
    }
    ...
    async fn unadvertise_channel(&self, channel: &Arc<Channel>) {
        if channel.schema.is_none() {
            tracing::error!(
                "Ignoring unadvertise channel for {} because a schema is required",
                channel.topic
            );
            return;
        }

        self.channels.write().remove(&channel.id);

        let message = protocol::server::unadvertise(channel.id);
        let clients = self.clients.get();
        for client in clients.iter() {
            if let Err(err) = client
                .control_plane_tx
            ...
            while let Some(msg) = ws_receiver.next().await {
                match msg {
                    Ok(Message::Close(_)) => {
                        ...
                        new_client.cancel_token.cancel();
                    }
                    Ok(msg) => {
...
impl LogSink for Server {
    ...
    fn add_channel(&self, channel: &Arc<Channel>) {
        if channel.schema.is_none() {
            tracing::info!(
                "websocket server ignoring add channel for {} because a schema is required",
                channel.topic
            );
            return;
        }
    ...
    fn remove_channel(&self, channel: &Arc<Channel>) {
        if channel.schema.is_none() {
            tracing::info!(
                "websocket server ignoring remove channel for {} because a schema is required",
                channel.topic
            );
            return;
        }
        ...
        let ch = channel.clone();
        self.runtime_handle
            .spawn(async move { server.unadvertise_channel(&ch).await });
    }
}
