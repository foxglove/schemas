...
/// A wrapper to store an Arc<Channel> in a HashSet by topic.
#[derive(Clone)]
...
impl LogContext {
    ...
    pub fn remove_channel(&self, channel: &Arc<Channel>) {
        {
            let mut channels = self.channels.write();
            channels.remove(channel.topic.as_str());
        }
        self.sinks.for_each(|sink| {
...
impl Drop for LogContext {
    fn drop(&mut self) {
        for channel in self.channels.get_mut().drain() {
            self.sinks.for_each(|sink| {
...
mod tests {
    ...
    fn test_log_calls_sinks() {
        ...
        assert!(Arc::ptr_eq(&recorded1[0].channel, &channel));
        ...
        assert!(Arc::ptr_eq(&recorded2[0].channel, &channel));
