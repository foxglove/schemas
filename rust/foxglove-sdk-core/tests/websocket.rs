...
use foxglove_sdk_core::{collection, Channel, LogContext, LogSink, Metadata, Schema};
...
fn new_channel(topic: String) -> Arc<Channel> {
    Channel::new(
        topic,
...
async fn test_advertise_to_client() {
    ...
    let ch = new_channel("/foo".to_string());
...
async fn test_log_only_to_subscribers() {
    ...
    let ch1 = new_channel("/foo".to_string());
    let ch2 = new_channel("/bar".to_string());
    let ch3 = new_channel("/baz".to_string());

    ctx.add_channel(ch1.clone()).unwrap();
    ctx.add_channel(ch2.clone()).unwrap();
    ctx.add_channel(ch3.clone()).unwrap();
