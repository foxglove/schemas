...
fn generate_impls(out_dir: &Path, fds: &FileDescriptorSet) -> anyhow::Result<()> {
    ...
    result = result.and(writeln!(
        module,
        "use crate::schemas::{{descriptors, foxglove::*, TypedMessage}};"
    ));
    result = result.and(writeln!(module, "use prost::bytes::BufMut;"));
    result = result.and(writeln!(module, "use crate::typed_channel::TypedChannel;"));
    result = result.and(writeln!(module, "use foxglove_sdk_core::Schema;"));
    result = result.and(writeln!(module, "use std::borrow::Cow;"));
    ...
    fn get_schema() -> Schema {{
        Schema::new(
            \"foxglove.{name}\".to_string(),
            Some(\"protobuf\".to_string()),
            Cow::Borrowed(descriptors::{descriptor_name}),
        )
    }}
...
pub type {name}Channel = TypedChannel<{name}>;"
        )
