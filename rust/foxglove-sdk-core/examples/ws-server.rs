...
use clap::Parser;
use foxglove_sdk_core::websocket::{
    self, create_server_with_internal_options, Capability, ServerListener,
};
use foxglove_sdk_core::{collection, nanoseconds_since_epoch, Channel, LogSink, Metadata, Schema};
use serde_json::json;
use std::borrow::Cow;
...
async fn main() {
    ...
    match server.start(&args.host, args.port).await {
        Ok(addr) => log::info!("Server started at {addr}"),
        Err(e) => {
    ...
    let async_task = async {
        let channel = Arc::new(Channel::new(
            ...
            Some(Schema::new(
                ...
                Cow::Borrowed(br#"{"$schema":"http://json-schema.org/draft-07/schema#","type":"object","required":["msg","count"],"properties":{"msg":{"type":"string","description":"A message string"},"count":{"type":"number","description":"A numeric count value"}},"additionalProperties":false}"#),
            )),
