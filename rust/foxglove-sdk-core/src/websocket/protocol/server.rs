...
pub fn advertisement(channel: &Arc<Channel>) -> Result<String, FoxgloveError> {
    let schema = &channel.schema.as_ref().ok_or_else(|| {
        FoxgloveError::Fatal("a schema is required for WebSocket protocol".to_string())
    })?;

    let schema_data = match schema.encoding.as_deref() {
        Some("protobuf") => BASE64_STANDARD.encode(&schema.data),
    ...
    Ok(json!({
        ...
        "channels": [Advertisement{
            ...
            schema: schema_data,
            schema_encoding: schema.encoding.as_deref(),
        }],
    })
    .to_string())
}
